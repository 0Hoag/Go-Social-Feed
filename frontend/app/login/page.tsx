...
export default function LoginPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setUser(user);
            toast.success(language === 'vi' ? 'Đăng nhập thành công!' : 'Signed in successfully!');
            router.push('/feed');
        } catch (err: any) {
            toast.error(err?.response?.data?.message || (language === 'vi' ? 'Sai thông tin đăng nhập' : 'Invalid credentials'));
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="min-h-screen bg-black flex">
            ...
            <div className="hidden lg:flex lg:w-1/2 xl:w-[55%] bg-zinc-950 items-center justify-center relative overflow-hidden">
                ...
                <div className="relative z-10 max-w-md px-8 text-center">
                    ...
                    <h2 className="text-4xl font-bold text-white mb-4 leading-tight">
                        {language === 'vi' ? 'Bảo mật thông minh.' : 'Smart Security.'}<br />
                        <span className="text-sky-400">{language === 'vi' ? 'Đầu tư an tâm.' : 'Safe Investing.'}</span>
                    </h2>
                    <p className="text-zinc-500 text-lg leading-relaxed">
                        {language === 'vi'
                            ? 'Phân tích Smart Contract trong 30 giây. Bảo vệ bạn khỏi Rug Pull và Honeypot.'
                            : 'Analyze Smart Contracts in 30 seconds. Protect yourself from Rug Pulls and Honeypots.'}
                    </p>

                    {/* Stats */}
                    <div className="mt-10 grid grid-cols-3 gap-4">
                        {[
                            { value: '50K+', label: language === 'vi' ? 'Hợp đồng quét' : 'Contracts scanned' },
                            { value: '99.2%', label: language === 'vi' ? 'Độ chính xác' : 'Accuracy' },
                            { value: '30s', label: language === 'vi' ? 'Kết quả' : 'Results in' },
                        ].map((s) => (
                            <div key={s.label} className="bg-zinc-800/50 border border-zinc-800 rounded-2xl p-4">
            ...
            <div className="flex-1 flex items-center justify-center p-6 lg:p-12 relative">
                <div className="w-full max-w-sm">
                    ...
                    <h1 className="text-3xl font-bold text-white mb-1">{a.login_title}</h1>
                    <p className="text-zinc-500 mb-8">{a.login_subtitle}</p>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="w-full bg-sky-500 hover:bg-sky-400 disabled:opacity-60 disabled:cursor-not-allowed text-white font-semibold py-3 rounded-xl transition-all duration-200 shadow-lg shadow-sky-500/20 flex items-center justify-center gap-2 text-sm mt-2"
                        >
                            {loading ? (
                                <><svg className="animate-spin w-4 h-4" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>{a.logging_in}</>
                            ) : a.login_btn}
                        </button>
                    </form>
                    ...
                    <p className="mt-6 text-center text-sm text-zinc-500">
                        {a.no_account}{' '}
                        <Link href="/register" className="text-sky-400 hover:text-sky-300 font-medium transition-colors">{a.register_link}</Link>
                    </p>
                </div>
            </div>
        </div>
