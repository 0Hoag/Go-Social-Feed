...
export default function AnalysisPage() {
    ...
    useEffect(() => {
        ...
        if (selectedSymbol !== "XAUUSD") {
            ...
            ws.onmessage = (event) => {
                ...
                // Mark data as ready when first order book data arrives
                if (!isDataReady) {
                    setIsDataReady(true);
                }
            };
    ...
    }, [selectedSymbol, isDataReady]);

    // Handle loading completion - called after minimum display time
    const handleLoadingComplete = () => {
        // Always hide loading screen after timer completes
        setIsLoading(false);
    };

    // Also hide loading if data is ready (in case it loads very quickly)
    useEffect(() => {
        if (isDataReady) {
    ...
    return (
        <>
            ...
            <main className={`min-h-screen bg-[#050505] text-gray-200 p-4 transition-opacity duration-500 ${isLoading ? 'opacity-0' : 'opacity-100'
                }`}>
